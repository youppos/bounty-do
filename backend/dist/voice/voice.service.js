...
let VoiceService = class VoiceService {
    parseSpeech(text) {
        ...
        const levelRegex = /(?:level|priority|等级|优先级|重要度|重要性|重要级别)\s*([1-5])/i;
        ...
        if (levelMatch) {
            ...
            if (checkKeywords(legendaryKeywords))
                levelIndex = 4;
            else if (checkKeywords(epicKeywords))
                levelIndex = 3;
            else if (checkKeywords(hardKeywords))
                levelIndex = 2;
            else if (checkKeywords(normalKeywords))
                levelIndex = 1;
            else if (checkKeywords(easyKeywords))
                levelIndex = 0;
        }
        ...
        if (dayAfterTomorrowRegex.test(cleanText)) {
            deadlineDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 2);
        ...
        if (inDaysMatch && !dateFound) {
            ...
            deadlineDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + daysOffset);
        ...
        if (weekDayIndex !== -1 && !dateFound) {
            ...
            deadlineDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + daysToAdd);
            dateFound = true;
        }
        let hour = 18;
        let minute = 0;
        let timeFound = false;
        const timeFullRegex = /(\d{1,2})[:：](\d{2})|(\d{1,2})\s*(?:点|点钟|hour)\s*(\d{2})?(?:分)?/i;
